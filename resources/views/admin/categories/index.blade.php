<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between">
            ...
            <div class="p-1-">
                <Link class="bg-blue-900 text-white rounded-xl  px-4 py-3" modal  href="{{ route('admin.categories.create') }}">
                 Add Category
                </Link>
            </div>
        </div>
                ...
                <x-splade-lazy>
                    ...
                    <x-splade-table class="mt-5" :for="$data">
                        ...
                        <x-splade-cell is_featured as="$item">
                            <Link method="POST" href="{{ route('api.category.toggle',['type' => 'featured', 'id' => $item->id]) }}">
                        ...
                        <x-splade-cell status as="$item">
                            <Link method="POST" href="{{ route('api.category.toggle',['type' => 'status', 'id' => $item->id]) }}">
                            <label class="relative inline-flex items-center mb-5 cursor-pointer">
